...
export default function JudgesPage({event, mode} : {event: Event | undefined, mode: boolean}) {
            ...
            <ManageTeamModal isOpen={modalOpen !== 0} onClose={() => setModalOpen(0)} mode={modalOpen} eventId={eventId!} teamData={selectedTeamData} />
                <div className="w-full space-y-3">
                    <div className="flex justify-between items-center gap-5">
                        <h1 className="lg:text-[22px] md:text-[20px] text-[18px] font-bold mb-0">{event?.title}'s Judges</h1>
                        {mode && <Button buttonText="New Judge" onClick={handleOpenCreateModal} type="primary" width="auto" buttonIcon={<FaPlus />} />}
                    </div>
