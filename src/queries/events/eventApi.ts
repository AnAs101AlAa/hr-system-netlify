...
export class eventsApi {
  ...
  async fetchPastEventsCount(eventType: string, title: string): Promise<number> {
    const nowDate = new Date().toISOString();
    const response = await systemApi.get<number>(`${EVENTS_API_URL}Events/filtered?toDate=${nowDate}&${eventType != "" ? `eventType=${eventType}` : ""}&${title != "" ? `title=${title}` : ""}`);
    return response.data.data.items.length;
  }
  ...
  async fetchPastEvents(eventType: string, title: string, page: number, pageSize: number): Promise<Event[]> {
    const nowDate = new Date().toISOString();
    const response = await systemApi.get<Event[]>(`${EVENTS_API_URL}Events/filtered?toDate=${nowDate}&${eventType != "" ? `eventType=${eventType}` : ""}&${title != "" ? `title=${title}` : ""}&page=${page}&count=${pageSize}`);
    return response.data;
  }
}
